<script lang="ts">
	...
	let label = $derived(type.replace(/_/g, ' '));
	let style = $derived(EVENT_STYLES[type] ?? 'bg-[#64748b] text-white');
</script>

<span class="min-w-[70px] rounded px-1 py-0.5 text-center text-[10px] font-bold uppercase {style}">
	{label}
</span>
