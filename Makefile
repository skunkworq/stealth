...
$(LABD_BINARY): rust-sniffer $(shell find brws/lab brws/adversarial cmd/lab/server/labd -name '*.go' 2>/dev/null) brws/fingerprint/lab/static/index.html
	@echo "Building labd..."
	@mkdir -p $(BINARY_DIR)
	go build $(LDFLAGS) -o $@ $(LABD_SRC)
	@echo "✓ Built $@"

$(BRWSLAB_BINARY): $(shell find brws -name '*.go' cmd/lab/cli/brwslab -name '*.go' 2>/dev/null)
