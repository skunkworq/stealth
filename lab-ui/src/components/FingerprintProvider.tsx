...
const platformPresets: Record<string, {
  ...
  http2: { header_table_size: number; initial_window_size: number; max_concurrent_streams: number };
}> = {
  windows: {
    name: "Windows 10/11",
    tls: {
      ...
      extensions: [
        "server_name", "extended_master_secret", "supported_groups", "ec_point_formats",
        "signature_algorithms", "application_layer_protocol_negotiation", "status_request",
        "signed_certificate_timestamp", "key_share", "supported_versions", "compress_certificate",
...
const browserPresets: Record<string, {
  name: string;
  user_agent: string;
  headers: string[];
  ...
  chrome: {
    ...
    headers: ["sec-ch-ua", "sec-ch-ua-mobile", "sec-ch-ua-platform", "upgrade-insecure-requests", "user-agent", "accept", "sec-fetch-site", "sec-fetch-mode", "sec-fetch-user", "sec-fetch-dest", "accept-encoding", "accept-language"],
    client_hints: true,
  },
  firefox: {
    name: "Mozilla Firefox",
    user_agent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:109.0) Gecko/20100101 Firefox/109.0",
    headers: ["user-agent", "accept", "accept-language", "accept-encoding", "upgrade-insecure-requests", "sec-fetch-dest", "sec-fetch-mode", "sec-fetch-site", "sec-fetch-user", "te"],
    client_hints: false,
  },
  safari: {
    name: "Apple Safari",
  ...
  if (Array.isArray(obj)) {
    ...
    return obj.map(item => {
      if (typeof item === "object" && item !== null) {
        const lines = toYAML(item, indent + 1).trimEnd().split("\n");
        return `${pad}- ${lines[0].trim()}\n${lines.slice(1).map(l => `${pad}  ${l.trim() ? `  ${l.trim()}` : ""}\n`).join("")}`;
      }
...
export function FingerprintProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { fetchFingerprint(); }, [fetchFingerprint]);

  const applyPlatformPresetFn = useCallback((key: string) => {
    ...
    if (updated.tls) {
      ...
      updated.tls.cipher_suites = preset.tls.ciphers.map((name, i) => ({ name, value: 0, position: i + 1, is_grease: false }));
      updated.tls.extensions = preset.tls.extensions.map((name, i) => ({ name, type: 0, position: i + 1, is_grease: false }));
    }
    if (updated.http2) {
  ...
  const applyBrowserPresetFn = useCallback((key: string) => {
    if (!fingerprint) return;
    ...
    if (updated.http) {
      ...
      updated.http.headers = preset.headers.map((name, i) => ({
        name, value: "", position: i + 1, is_pseudo: name.startsWith(":"),
      }));
      if (preset.client_hints) {
        ...
        updated.http.client_hints = null;
      }
    }
    setFingerprint(updated);
  ...
  const selectCapture = useCallback(async (id: string) => {
    if (selectedCaptureId === id) {
      // Deselect — go back to live capture
  ...
  }, [selectedCaptureId, fetchFingerprint]);
  ...
  const yamlOutput = useMemo(() => {
    if (!fingerprint) return "";
    try {
      return fingerprintToYAML(fingerprint);
    } catch {
  ...
  return (
    <FingerprintContext.Provider value={{
      fingerprint,
      originalFingerprint,
      loading,
      error,
      ...
      jsonOutput,
      yamlOutput,
      refresh: fetchFingerprint,
      selectCapture,
      selectedCaptureId,
      applyPlatformPreset: applyPlatformPresetFn,
      applyBrowserPreset: applyBrowserPresetFn,
      resetToOriginal,
      platformPresets,
      browserPresets,
    }}>
