...
// labTraceMetrics holds basic mouse/keyboard metrics derived from a CaptchaEvent slice.
type labTraceMetrics struct {
	TotalEvents      int
	MouseMoves       int
	Clicks           int
	Keystrokes       int
...
// computeRecordingMetrics derives behavioral metrics from a CaptchaEvent slice.
func computeRecordingMetrics(events []CaptchaEvent) *labTraceMetrics {
	m := &labTraceMetrics{TotalEvents: len(events)}
	if len(events) == 0 {
		return m
	}
	...
	if totalDist > 0 {
		dx := lastX - firstX
		dy := lastY - firstY
		displacement := labSqrt(dx*dx + dy*dy)
...
func labSqrt(x float64) float64 {
	if x <= 0 {
		return 0
	}
	r := x
