...
type CaptchaShield struct {
	...
	trainingData     *captchatraining.CaptchaTrainingData
	config           *CaptchaShieldConfig
	activeChallenges map[string]*captchatraining.CaptchaChallenge
...
// Type aliases that delegate to captchatraining so all packages share one canonical definition.
type (
	CaptchaChallenge = captchatraining.CaptchaChallenge
	ChallengeMetrics = captchatraining.ChallengeMetrics
	CaptchaEvent     = captchatraining.CaptchaEvent
	TraceMetrics     = captchatraining.TraceMetrics
)

// NewCaptchaShield creates a new CAPTCHA shield with the given configuration.
func NewCaptchaShield(config *CaptchaShieldConfig, detector *challenge.Detector, tracer *captchatraining.CaptchaTracer) *CaptchaShield {
	if config == nil {
	...
	return &CaptchaShield{
		...
		tracer:           tracer,
		trainingData:     captchatraining.NewCaptchaTrainingData(),
		config:           config,
		activeChallenges: make(map[string]*captchatraining.CaptchaChallenge),
		detector:         detector,
	}
}

// ShouldPresentCaptcha determines whether to present a CAPTCHA based on detection results.
func (cs *CaptchaShield) ShouldPresentCaptcha(detection *detection.DetectionResult) bool {
	if detection == nil {
		return false
	}
...
func (cs *CaptchaShield) CreateChallenge(sessionID string, detection *detection.DetectionResult, captchaType string) (*captchatraining.CaptchaChallenge, error) {
	cs.mu.Lock()
	defer cs.mu.Unlock()

	challenge := &captchatraining.CaptchaChallenge{
...
func (cs *CaptchaShield) selectTargetFromDetection(detection *detection.DetectionResult) string {
	targets := []string{
...
func (cs *CaptchaShield) selectCategoryFromDetection(detection *detection.DetectionResult) string {
	categories := []string{
...
func (cs *CaptchaShield) GetChallenge(challengeID string) (*captchatraining.CaptchaChallenge, bool) {
	cs.mu.RLock()
	defer cs.mu.RUnlock()
...
func (cs *CaptchaShield) GetActiveChallenges() []*captchatraining.CaptchaChallenge {
	cs.mu.RLock()
	defer cs.mu.RUnlock()

	challenges := make([]*captchatraining.CaptchaChallenge, 0, len(cs.activeChallenges))
...
func (cs *CaptchaShield) GetTrainingData() *captchatraining.CaptchaTrainingData {
...
func NewCaptchaShieldMiddleware(shield *CaptchaShield, detector *challenge.Detector) *CaptchaShieldMiddleware {
...
func (m *CaptchaShieldMiddleware) HandleDetection(sessionID string, detection *detection.DetectionResult) (*captchatraining.CaptchaChallenge, bool) {
...
func (m *CaptchaShieldMiddleware) selectCaptchaType(detection *detection.DetectionResult) string {
	if detection == nil {
		return "text"
	}
