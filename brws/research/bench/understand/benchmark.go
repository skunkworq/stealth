...
func NewSuite(config *understand.PipelineConfig, cache *understand.CacheStore) *Suite {
	return &Suite{
		config: config,
		cache:  cache,
		client: &http.Client{
...
func (s *Suite) RunURL(ctx context.Context, url string) *Result {
	...
	start := time.Now()
	tree, stats, err := understand.HTMLToSemanticTreeCached(ctx, html, url, s.config)
	if err != nil {
		result.Error = err.Error()
		return result
	}
...
func nodeDepth(node understand.SemanticNode, depth int) int {
	maxDepth := depth
