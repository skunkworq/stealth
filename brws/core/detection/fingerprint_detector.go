// Package challenge provides tools to detect browser fingerprint spoofing.
...
// NewAnalyzer creates a new adversarial detector.
...
func (d *Analyzer) LoadBaseline(name string, fp *types.CompleteFingerprint) {
	d.mu.Lock()
	defer d.mu.Unlock()
	d.baselines[name] = fp
}
...
func (d *Analyzer) AnalyzeTLS(clientHello *tls.ClientHelloInfo) *TLSAnalysis {
...
func (d *Analyzer) AnalyzeHTTP(req *http.Request) *HTTPAnalysis {
...
func (d *Analyzer) DetectFromJA3(ja3 string) (bool, float64) {
...
func (d *Analyzer) DetectFromHeaders(headers http.Header) (bool, float64) {
	score := 0.0
...
func (d *Analyzer) CompareToBaseline(name string, fp *types.CompleteFingerprint) *DetectionResult {
	d.mu.RLock()
...
func (d *Analyzer) AddResult(result DetectionResult) {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *Analyzer) GetResults() []DetectionResult {
	d.mu.RLock()
	defer d.mu.RUnlock()
	return d.results
}
...
func (d *Analyzer) detectTLSAnomalies(analysis *TLSAnalysis) []string {
...
func (d *Analyzer) detectHTTPAnomalies(analysis *HTTPAnalysis) []string {
	anomalies := make([]string, 0)
