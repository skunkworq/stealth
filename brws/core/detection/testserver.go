...
func (ts *TestServer) handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	_, headersScore := analyzer.DetectFromHeaders(r.Header)
	...
	record.IsBot = record.Score > 0.3 //nolint:mnd
	...
	if record.IsBot {
		resp["detection_details"] = map[string]interface{}{
...
func RunTestSuite(tests []struct {
	...
	client := &http.Client{
		Transport: &http.Transport{
			TLSClientConfig: &tls.Config{ //nolint:gosec // G402: InsecureSkipVerify required for test client
	...
	for _, test := range tests {
		...
		if err != nil {
			results = append(results, TestResult{TestName: test.Name, Passed: false, Error: err})
			continue
		}
...
func ExpectHuman(headers http.Header) (bool, float64, []string) {
	...
	analyzer := NewAnalyzer()
	isBot, score := analyzer.DetectFromHeaders(headers)
	...
	totalScore := (score + httpResult.Score) / 2 //nolint:mnd
