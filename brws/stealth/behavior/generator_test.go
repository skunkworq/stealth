...
func TestEventGenerator_PassesShieldAnalysis(t *testing.T) {
	...
	gen := NewEventGenerator(nil)
	ba := challenge.NewBehavioralAnalyzer(nil)
	...
	for i := 0; i < 20; i++ {
		...
		// Convert to challenge.EnhancedBehavioralEvents
		events := &challenge.EnhancedBehavioralEvents{
		...
		for _, pos := range data.MousePositions {
			events.MousePositions = append(events.MousePositions, challenge.Position{
				X: pos["x"],
				Y: pos["y"],
			})
		}
		...
		for _, pos := range data.ClickPositions {
			events.ClickPositions = append(events.ClickPositions, challenge.Position{
				X: pos["x"],
				Y: pos["y"],
			})
		}
...
func TestEventGenerator_P10_PassesNewShieldChecks(t *testing.T) {
	...
	for i := 0; i < trials; i++ {
		...
		for _, pos := range data.MousePositions {
			events.MousePositions = append(events.MousePositions, challenge.Position{X: pos["x"], Y: pos["y"]})
		}
		for _, pos := range data.ClickPositions {
			events.ClickPositions = append(events.ClickPositions, challenge.Position{X: pos["x"], Y: pos["y"]})
		}
