...
func (ud *UnifiedDetector) Detect(resp *engine.Response) *challenge.Challenge {
	...
	// 2. Cloudflare detection (via adversarial.DetectChallenge)
	...
	cfChallenge := adversarial.DetectChallenge(resp.Status, httpHeaders, resp.Body)
	if cfChallenge != nil {
...
func cfChallengeTypeToGeneric(cfType adversarial.CloudflareChallengeType) challenge.ChallengeType {
	switch cfType {
