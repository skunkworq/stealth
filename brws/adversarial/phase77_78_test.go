...
func TestPhases77_78Integration(t *testing.T) {
	...
	if !hasGamepad { t.Errorf("Phase 77 detection failed: missing gamepad indicator in %v", fired1) }
	if !hasBluetooth { t.Errorf("Phase 78 detection failed: missing bluetooth indicator in %v", fired1) }
	if !hasUSB { t.Errorf("Phase 78 detection failed: missing usb indicator in %v", fired1) }
