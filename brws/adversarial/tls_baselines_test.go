...
func TestTLSDeepAnalysis_GoDefault(t *testing.T) {
	goBaseline := adversarial.GoDefaultBaseline()
	...
	analysis := adversarial.AnalyzeTLSDeep(fp, "chrome")
	t.Logf("Go Default Analysis:\n%s", adversarial.FormatTLSAnalysis(analysis))
...
func TestTLSDeepAnalysis_Chrome146(t *testing.T) {
	baseline := adversarial.Chrome146Baseline()
...
func TestTLSDeepAnalysis_Firefox128(t *testing.T) {
	baseline := adversarial.Firefox128Baseline()
	fp := buildFingerprintFromBaseline(baseline)

	analysis := adversarial.AnalyzeTLSDeep(fp, "firefox")
...
func TestTLSDeepAnalysis_CapturedTrainingData(t *testing.T) {
	...
	analysis := adversarial.AnalyzeTLSDeep(tlsFP, "chrome")
	...
	reports := adversarial.CompareTLSToBaselines(tlsFP)
...
func TestTLSBaselineComparison(t *testing.T) {
	baselines := []*adversarial.TLSReferenceBaseline{
		adversarial.Chrome146Baseline(),
		adversarial.Firefox128Baseline(),
		adversarial.GoDefaultBaseline(),
	}

	for _, bl := range baselines {
		t.Run(bl.Browser+"_"+bl.Version, func(t *testing.T) {
			fp := buildFingerprintFromBaseline(bl)
			analysis := adversarial.AnalyzeTLSDeep(fp, bl.Browser)
			...
			reports := adversarial.CompareTLSToBaselines(fp)
			for _, r := range reports {
...
func TestTLSShieldIntegration(t *testing.T) {
	...
	t.Run("go_tls_chrome_ua", func(t *testing.T) {
		goFP := buildFingerprintFromBaseline(adversarial.GoDefaultBaseline())
	...
	t.Run("chrome_tls_chrome_ua", func(t *testing.T) {
		chromeFP := buildFingerprintFromBaseline(adversarial.Chrome146Baseline())
...
func buildFingerprintFromBaseline(bl *adversarial.TLSReferenceBaseline) *types.TLSFingerprint {
	fp := &types.TLSFingerprint{
		Version:     0x0303,
		VersionName: "TLS 1.2",
		ALPN:        bl.ALPN,
