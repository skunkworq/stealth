...
func TestSwordVsShield(t *testing.T) {
	...
	t.Run("library_matrix_comparison", func(t *testing.T) {
		...
		// Armed sword: the shield should catch the current navigation evasion path.
		...
		if browserResult.DetectionRate < 0.95 {
			t.Fatalf("expected browser sword detection >= 95%% in library matrix, got %.0f%%",
				browserResult.DetectionRate*100)
		}
		if browserResult.AvgBotScore < 0.50 {
			t.Fatalf("expected browser sword avg score >= 0.50 in library matrix, got %.3f", browserResult.AvgBotScore)
		}
	})
