...
func TestPhases81_82Integration(t *testing.T) {
	...
	if !hasRendererMismatch { t.Errorf("Phase 81 detection failed: missing renderer_platform_mismatch") }
	if !hasAudioStateMismatch { t.Errorf("Phase 82 detection failed: missing audio_context_static_state") }
	if !hasAudioLatencyMismatch { t.Errorf("Phase 82 detection failed: missing audio_latency_improbable") }
