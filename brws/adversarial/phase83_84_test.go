...
func TestPhases83_84Integration(t *testing.T) {
	...
	if !hasMemoryMismatch { t.Errorf("Phase 83 detection failed: missing performance_memory_limit_mismatch") }
	if !hasNavTypeMismatch { t.Errorf("Phase 83 detection failed: missing performance_navigation_type_mismatch") }
	if !hasNavStartInconsistent { t.Errorf("Phase 84 detection failed: missing timing_navigation_start_inconsistent") }
	if !hasLoadEventInconsistent { t.Errorf("Phase 84 detection failed: missing timing_load_event_inconsistent") }
