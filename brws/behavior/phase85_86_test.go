...
func TestPhases85_86Integration(t *testing.T) {
	...
	for _, v := range report1.Vectors {
		for _, c := range v.Checks {
			if c.Fired {
				if c.Name == "touch_pointer_mismatch" { hasTouchMismatch = true }
				if c.Name == "missing_orientation_lock" { hasOrientationMismatch = true }
			}
		}
	}

	if !hasTouchMismatch { t.Errorf("Phase 85 detection failed: missing touch_pointer_mismatch") }
	if !hasOrientationMismatch { t.Errorf("Phase 86 detection failed: missing missing_orientation_lock") }
	...
	for _, v := range report2.Vectors {
		for _, c := range v.Checks {
			if c.Fired {
				if c.Name == "touch_pointer_mismatch" { t.Errorf("Phase 85 hardening failed: touch_pointer_mismatch still fires") }
				if c.Name == "missing_orientation_lock" { t.Errorf("Phase 86 hardening failed: missing_orientation_lock still fires") }
			}
		}
	}
