...
// With caching:
//
//	cache, _ := understand.NewCacheStore(ctx, "")
//	defer cache.Close()
//	config.Cache = cache
//
...
//
//	cache, _ := understand.NewCacheStore(ctx, "~/.semantic/cache.db")
//	defer cache.Close()
//
...
//
//	forms := understand.ExtractFormSchemas(html)
//	// []FormSchema with:
...
//
//	grounding := understand.ExtractVisualGrounding(html, 1920, 1080)
...
//
//	diff := understand.ComputeDiff(oldTree, newTree)
...
//
//	updater := understand.NewIncrementalUpdater(cache, config)
...
//
//	serialized := understand.SerializeTree(tree, state, nil)
//	// [WEBFURL] format with hierarchical structure
//
// Budget-aware unfolding:
//
//	state := understand.InitialPack(tree, 4000, 16000)
//	tree, _ := understand.HTMLToSemanticTreeCached(ctx, html, url, config)
//	understand.AutoUnfold(tree, state)
//
...
//
//	var err *understand.LLMError
//	if errors.As(err, &e) {
//	    // LLM API error
//	}
//
//	var err *understand.CacheError
//	if errors.As(err, &e) {
//	    // SQLite cache error
//	}
//
