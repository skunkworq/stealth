...
type VectorIndex struct {
	mu       sync.RWMutex
	nodes    []*indexedNode
	embedder *understand.EmbeddingClient
}
...
func NewVectorIndex(embedder *understand.EmbeddingClient) *VectorIndex {
	return &VectorIndex{
...
func (idx *VectorIndex) Add(node *understand.SemanticNode, url string, embedding []float32) {
	idx.mu.Lock()
	defer idx.mu.Unlock()
...
func (idx *VectorIndex) AddTree(ctx context.Context, tree *understand.SemanticTree) error {
...
func (idx *VectorIndex) addNodeRecursive(ctx context.Context, node *understand.SemanticNode, url string) error {
...
func (idx *VectorIndex) FindSimilar(node *understand.SemanticNode, k int) []SearchResult {
	if len(node.Embedding) == 0 {
		return nil
	}
