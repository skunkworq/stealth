...
func NewPipeline(config *Config) (*Pipeline, error) {
	...
	if config.EnableCache {
		cache, err := semantic.NewCacheStore(context.Background(), "")
		if err != nil {
...
func (p *Pipeline) extractStage(ctx context.Context, url, html string) (*semantic.SemanticTree, *semantic.CompressionStats, error) {
	tc := TraceFromContext(ctx)
	var span *Span
	if tc != nil {
...
func (p *Pipeline) embedStage(ctx context.Context, tree *semantic.SemanticTree, url string) error {
	tc := TraceFromContext(ctx)
	var span *Span
	if tc != nil {
