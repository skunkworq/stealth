// Package chromestealth provides a stealth-enhanced Chromium engine.
//
		...
		case *network.EventRequestWillBeSent:
			...
			netRequests[e.RequestID] = &engine.TraceEntry{
				...
				Request: engine.TraceRequest{
					Headers: chromium.FlattenHeaders(e.Request.Headers),
				},
			}
			netMu.Unlock()
		case *network.EventResponseReceived:
			...
			if entry, ok := netRequests[e.RequestID]; ok {
				...
				entry.Response = engine.TraceResponse{
					Headers:  chromium.FlattenHeaders(e.Response.Headers),
